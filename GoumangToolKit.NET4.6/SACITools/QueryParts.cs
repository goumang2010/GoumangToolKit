        ...
        private static string[] QueryItems = new string[]
{
      ...
      static  MongoMethod mmlocal = new MongoMethod(client, database, col);
      static  MongoMethod mmFTP = new MongoMethod(client, database, FTP_col);
        ...
        public static Dictionary<string, string> queryData(string extension, string partNameQuery)
        {
            ...
            Func<string, bool?> pathfill = delegate (string partnum)
            {
                ...
                var query = new BsonDocument("FileName", c22);
                //var sortBy = SortBy.Descending("Rev");

                var dt_path = mmlocal.collection.Find(query).SortByDescending(p=>p["Rev"]);
                ...
                if (dt_path.Count() == 0)
                {
                    ...
                    query = new BsonDocument("FileName", c22);
                 //   sortBy = SortBy.Descending("Rev");

                    dt_path = mmFTP.collection.Find(query).SortByDescending(p => p["Rev"]);
                ...
                else
                {
                    ...
                    query = new BsonDocument { { "$and", ba } };

                    dt_path = mmlocal.collection.Find(query).SortByDescending(p => p["Rev"]);
